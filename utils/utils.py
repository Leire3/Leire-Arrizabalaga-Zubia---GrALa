...
def cycle_prediction(X,Y, dataset, input_, to_predict, n_features):
    ...
    plt.legend()
    #plt.show()
    plt.savefig(f'./results/cycle_prediction/grafikoa_{dataset}_{input_}_{to_predict}_{n_features}.png')

    return

def obtain_pivot_df(df_battery, df_filtered, bats, feature, n_features, to_predict):
...
def prediction(dataset, df_summary, df_battery, input_, to_predict, n_features, group):
    ...
    if input_ == 'knee-charge' or input_=='knee-discharge': # knee_knees - capacity_knee - knee-discharge
        df_pivot, output_col = obtain_pivot_df(df_battery, df_filtered, bats, 'knee', n_features, to_predict)
    elif input_ == 'cut-of-voltage':
        df_pivot, output_col = obtain_pivot_df(df_battery, df_filtered, bats, 'Q_value', n_features, to_predict)
